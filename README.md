...
PlayerHealthUI.cs	玩家HUD：等级+血条+经验条PlayerHealthUI.cs	玩家HUD：等级 血条 经验条
...
SaveManager.cs	存档系统：PlayerPrefs+JSON存读，S保存/L读取/Esc回主页SaveManager.cs	存档系统：PlayerPrefs JSON存读，S保存/L读取/Esc回主页
MainMenu.cs	主菜单：新游戏/继续/退出
ItemPickUp.cs	拾取物品，触碰后入背包并装备
核心关联关系
Singleton（基类）
  ├── MouseManager ──事件──→ PlayerController ──攻击──→ CharacterStats（受伤计算）├── MouseManager ──事件──→ PlayerController ──攻击──→ CharacterStats（受伤计算）
  ├── GameManager  ←─注册── PlayerController├── GameManager  ←─注册── PlayerController
  │     └── 观察者列表 → EnemyController（EndNotify）│     └── 观察者列表 → EnemyController（EndNotify）
  │                    → SceneController（EndNotify→回主菜单）│                    → SceneController（EndNotify→回主菜单）
  ├── InventoryManager ←─── DragItem / SlotHolder / ContainerUI / ItemUI├──InventoryManager &larr；───DragItem / sloholder / ContainerUI / ItemUI
  ├── SaveManager    ←─── SceneController / MainMenu / PlayerController├──SaveManager &larr；──SceneController / MainMenu / PlayerController
  └── SceneController ←─── TransitionPoint（E键传送）└── SceneController ←─── TransitionPoint（E键传送）
